...
function preparePrescriptionForStorage(prescription) {
  ...
  return {
    ...
    prescriberLicense: normalizeText(prescription.prescriberLicense) || normalizeText(prescription.doctorId),
    items,
...
class PostgresDatabase {
  ...
  async getPatientHistory(patientId) {
    ...
    const prescriptionsResult = await this.pool.query(
      `
        SELECT
          prescription_id AS "prescriptionId",
          doctor_id AS "doctorId",
          ...
          items
        FROM valid_prescriptions
        WHERE LOWER(patient_id) = LOWER($1)
    ...
    return {
      prescriptions: prescriptionsResult.rows.map((prescription) => withDispensingFields(prescription)),
...
class JsonFileDatabase {
  ...
  async savePrescription(prescription) {
    ...
    if (existingIndex >= 0) {
      ...
      this.state.validPrescriptions[existingIndex] = {
        ...
        createdAt: this.state.validPrescriptions[existingIndex].createdAt ?? new Date().toISOString()
      };
    } else {
  ...
  async getPatientHistory(patientId) {
    const normalizedPatientId = normalizeText(patientId).toLowerCase();
    const prescriptions = this.state.validPrescriptions
      .filter((prescription) => normalizeText(prescription.patientId).toLowerCase() === normalizedPatientId)
      .map((prescription) => withDispensingFields(prescription))
      .sort((a, b) => new Date(b.createdAt ?? b.expiryDate).getTime() - new Date(a.createdAt ?? a.expiryDate).getTime());
    const transactions = this.state.transactions
      .filter((transaction) => normalizeText(transaction.patientId).toLowerCase() === normalizedPatientId)
      .sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime() || b.id - a.id);

    return clone({ prescriptions, transactions });
  }
