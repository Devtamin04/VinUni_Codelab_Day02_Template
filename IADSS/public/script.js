...
const doctorPatientIdInput = document.querySelector('#doctorPatientId');
const mainDiagnosisInput = document.querySelector('#mainDiagnosis');
const icd10CodeInput = document.querySelector('#icd10Code');
...
function hideDiagnosisSuggestions() {
  diagnosisSuggestions?.classList.remove('visible');
}

function renderDiagnosisSuggestions(results) {
  ...
  if (currentDiagnosisResults.length === 0) {
    diagnosisSuggestions.innerHTML = `
      <div class="diagnosis-option" role="option">
        <strong>No ICD-10 suggestion</strong>
        <span>Try another diagnosis term.</span>
      </div>
    `;
    diagnosisSuggestions.classList.add('visible');
    return;
  }

  diagnosisSuggestions.innerHTML = currentDiagnosisResults
    .map((diagnosis, index) => `
      <button class="diagnosis-option" type="button" role="option" data-diagnosis-index="${index}">
        <strong>${escapeHtml(diagnosis.code)} - ${escapeHtml(diagnosis.label)}</strong>
        <span>${escapeHtml((diagnosis.terms ?? []).slice(0, 3).join(', '))}</span>
      </button>
    `)
    .join('');
  diagnosisSuggestions.classList.add('visible');
...
async function searchDiagnoses(query) {
  ...
  try {
    const response = await apiFetch(`/api/clinical/diagnoses?q=${encodeURIComponent(trimmed)}`);

    if (!response.ok) {
      throw new Error('Diagnosis lookup failed');
    }

    const data = await response.json();
    renderDiagnosisSuggestions(data.suggestions ?? []);
  } catch (error) {
    hideDiagnosisSuggestions();
  }
}
...
function daysSince(timestamp) {
  ...
  return Number.isFinite(time) ? Math.floor((Date.now() - time) / (24 * 60 * 60 * 1000)) : Number.POSITIVE_INFINITY;
}
...
function collectHistoryEvents(history) {
  const prescriptions = (history?.prescriptions ?? []).flatMap((prescription) => {
    return getPrescriptionItems(prescription).map((item) => ({
      type: 'Prescription',
      timestamp: prescription.createdAt ?? prescription.expiryDate,
      prescriptionId: prescription.prescriptionId,
      hospitalName: prescription.hospitalName,
      drugName: getItemName(item),
      drugClass: item.drugClass ?? item.antibioticClass,
  ...
  return [...prescriptions, ...transactions]
    .filter((event) => event.drugName || event.drugClass)
    .sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
}

function buildClientDecisionSupport(payload, history) {
  const items = payload.items ?? [];
  const historyEvents = collectHistoryEvents(history);
  const recentEvents = historyEvents.filter((event) => daysSince(event.timestamp) <= 180);
  const allergyText = [payload.drugAllergies, ...(history?.prescriptions ?? []).map((item) => item.drugAllergies)]
  ...
  items.forEach((item) => {
    const drugName = normalizeClinicalText(item.drugName);
    const drugClass = normalizeClinicalText(item.drugClass);

    if (drugName && allergyText.includes(drugName)) {
      warnings.push({ severity: 'high', message: `Allergy history directly mentions ${item.drugName}. Review before saving.` });
    }

    if (allergyText.includes('penicillin') || allergyText.includes('amoxicillin') || allergyText.includes('ampicillin')) {
      if (drugClass.includes('penicillin')) {
        warnings.push({ severity: 'high', message: `${item.drugName} is in a penicillin class while allergy history mentions penicillin.` });
      }

      if (drugClass.includes('cephalosporin')) {
        warnings.push({ severity: 'medium', message: `${item.drugName} is a cephalosporin; allergy history mentions penicillin/beta-lactam risk.` });
      }
    }

    const repeatedDrug = recentEvents.find((event) => normalizeClinicalText(event.drugName) === drugName);
    const repeatedClass = recentEvents.find((event) => normalizeClinicalText(event.drugClass) === drugClass && daysSince(event.timestamp) <= 90);

    if (repeatedDrug) {
      warnings.push({ severity: 'medium', message: `${item.drugName} appears in recent patient history (${repeatedDrug.prescriptionId || 'N/A'}).` });
    } else if (repeatedClass) {
      warnings.push({ severity: 'low', message: `${item.drugClass} appears in the last 90 days. Review repeated class exposure.` });
    }
  });

  if (recentEvents.length >= 3) {
    warnings.push({ severity: 'medium', message: `Patient has ${recentEvents.length} antibiotic record(s) in the last 6 months.` });
  }

  return {
    warnings: warnings.filter((warning, index) => warnings.findIndex((item) => item.message === warning.message) === index),
    recentEvents
  };
}

function renderClinicalSupport(decisionSupport = {}, history = currentPatientHistory) {
  if (!clinicalSupportBody) {
    return;
  }

  const warnings = decisionSupport.warnings ?? [];
  const events = decisionSupport.recentEvents ?? collectHistoryEvents(history).filter((event) => daysSince(event.timestamp) <= 180);
  const allergyValues = (decisionSupport.knownAllergies ?? (history?.prescriptions ?? []).map((item) => item.drugAllergies))
    .map((item) => String(item ?? '').trim())
    .filter(Boolean);
  const warningHtml = warnings.length === 0
    ? '<div class="support-alert low"><strong>No active warning from current inputs.</strong><span class="support-meta">Still verify allergies and local clinical policy.</span></div>'
    : warnings.map((warning) => `<div class="support-alert ${escapeHtml(warning.severity || 'low')}"><strong>${escapeHtml(warning.severity || 'info').toUpperCase()}</strong><span>${escapeHtml(warning.message)}</span></div>`).join('');
  const timelineHtml = events.length === 0
    ? '<div class="empty-cell">No antibiotic record in the last 6 months.</div>'
    : `<div class="timeline-list">${events.slice(0, 6).map((event) => `<div class="timeline-item"><strong>${escapeHtml(event.drugName || event.drugClass || 'Antibiotic')}</strong><span>${escapeHtml(event.type || 'Record')} - ${escapeHtml(event.prescriptionId || 'N/A')} - ${escapeHtml(event.status || 'N/A')}</span><span>${escapeHtml(event.timestamp ? new Date(event.timestamp).toLocaleDateString() : 'Unknown date')} - ${escapeHtml(event.hospitalName || 'Unknown location')}</span></div>`).join('')}</div>`;

  clinicalSupportBody.innerHTML = `
    ${warningHtml}
...
async function loadPatientHistory(patientId = doctorPatientIdInput?.value) {
  ...
  if (!normalized || !currentUser || !canAccessTab('doctorPanel')) {
  ...
  try {
    const response = await apiFetch(`/api/patients/${encodeURIComponent(normalized)}/history`);
    const data = await response.json();
    ...
    currentPatientHistory = null;
    clinicalSupportBody.innerHTML = '<div class="empty-cell">Unable to load patient history.</div>';
    return null;
  }
}

function refreshClinicalSupport() {
  if (!currentPatientHistory) {
    renderClinicalSupport({}, null);
    return;
  }

  renderClinicalSupport(buildClientDecisionSupport(getPrescriptionFormPayload(), currentPatientHistory), currentPatientHistory);
}
...
function populateTransactionForm(prescription, selectedItem = null) {
  ...
  if (form.elements.prescriberLicense) {
...
async function savePrescriptionFromForm(message = 'Prescription saved and sent to pharmacy registry.') {
  ...
  renderClinicalSupport(data.decisionSupport ?? data.prescription?.decisionSupport ?? {}, currentPatientHistory);
  setPrescriptionAlert(
    data.decisionSupport?.warningCount > 0 ? 'warning' : 'success',
    data.decisionSupport?.warningCount > 0 ? `${message} Review clinical decision support warnings.` : message
  );
...
document.addEventListener('mousedown', (event) => {
  ...
  if (!event.target.closest('.diagnosis-field')) {
    hideDiagnosisSuggestions();
  }
});
...
mainDiagnosisInput?.addEventListener('input', (event) => {
  window.clearTimeout(diagnosisLookupTimer);
  diagnosisLookupTimer = window.setTimeout(() => {
    searchDiagnoses(event.target.value);
  }, 250);
});

mainDiagnosisInput?.addEventListener('focus', () => {
  searchDiagnoses(mainDiagnosisInput.value);
});

diagnosisSuggestions?.addEventListener('mousedown', (event) => {
  const option = event.target.closest('[data-diagnosis-index]');
  ...
  event.preventDefault();
  const diagnosis = currentDiagnosisResults[Number(option.dataset.diagnosisIndex)];

  if (diagnosis) {
    mainDiagnosisInput.value = diagnosis.label;
    icd10CodeInput.value = diagnosis.code;
    hideDiagnosisSuggestions();
  }
});

doctorPatientIdInput?.addEventListener('blur', () => loadPatientHistory());
doctorPatientIdInput?.addEventListener('keydown', (event) => {
  if (event.key === 'Enter') {
    event.preventDefault();
    loadPatientHistory();
  }
...
drugAllergiesInput?.addEventListener('input', refreshClinicalSupport);
medicationItems?.addEventListener('input', refreshClinicalSupport);
medicationItems?.addEventListener('change', refreshClinicalSupport);
