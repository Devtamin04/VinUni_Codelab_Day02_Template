...
describe('IADSS API', () => {
  ...
  it('lets doctors save prescriptions without entering a prescriber license', async () => {
    const prescriptionId = nextPrescriptionId('RX-NO-LICENSE');
    const response = await request(app)
      .post('/api/prescriptions')
      .set(authHeader('doctor'))
      .send({
        prescriptionId,
        patientId: '77123',
        hospitalName: 'License-Free Clinic',
        antibioticName: 'Cefixime',
        antibioticClass: 'Cephalosporin',
        dosage: '200mg',
        quantityLimit: 10,
    ...
    assert.equal(response.body.prescription.doctorId, users.doctor.username);
    assert.equal(response.body.prescription.prescriberLicense, users.doctor.username);
  });
  ...
  it('suggests ICD-10 diagnoses and returns clinical decision support warnings', async () => {
    const suggestions = await request(app)
      .get('/api/clinical/diagnoses?q=sore%20throat')
      .set(authHeader('doctor'))
      .expect(200);

    assert.ok(suggestions.body.suggestions.some((item) => item.code === 'J02.9'));

    const previous = await saveAmoxicillinPrescription({
      prescriptionId: nextPrescriptionId('RX-HISTORY'),
      patientId: '99123',
      drugAllergies: 'Penicillin allergy'
    });
    ...
    const history = await request(app)
      .get('/api/patients/99123/history')
      .set(authHeader('doctor'))
      .expect(200);

    assert.equal(history.body.history.prescriptions.length, 1);
    assert.equal(history.body.history.transactions.length, 1);

    const warned = await request(app)
      .post('/api/prescriptions')
      .set(authHeader('doctor'))
      .send({
        prescriptionId: nextPrescriptionId('RX-CDS'),
        patientId: '99123',
        hospitalName: 'National General Hospital',
        prescriberLicense: '98765',
        mainDiagnosis: 'Acute pharyngitis',
    ...
    assert.ok(warned.body.decisionSupport.warningCount >= 1);
    assert.ok(warned.body.decisionSupport.warnings.some((warning) => warning.code === 'ALLERGY_RELATED_CLASS'));
  });
